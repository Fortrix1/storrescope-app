...
async function safeFetch(url) {
  try {
    return await fetch(url, { headers: HEADERS, redirect: 'follow', signal: AbortSignal.timeout(8000) })
  } catch { return null }
}
...
async function analyseStore(url) {
  ...
  const [seoData, productData, trustData, psData] = await Promise.all([
    checkSEO(base, html), checkProducts(base), checkTrust(base, html), getPageSpeed(base),
  ])
  ...
  const storeName  = titleMatch ? titleMatch[1].split('–')[0].split('|')[0].trim().slice(0, 50) : base.replace(/https?:\/\//, '').split('.')[0]
  ...
  const criticalData = buildCriticals({
    has_meta_description: seoData.has_meta_description, has_meta_title: seoData.has_meta_title,
    has_reviews: trustData.has_reviews, has_return_policy: trustData.has_return_policy,
    has_social: trustData.has_social, pagespeed_score: pagespeedScore,
    product_count: productCount, avg_images: productData?.avg_images || 0, redesign_needed: redesignNeeded,
  })

  return {
    store_name: storeName, platform, niche, is_new_seller: isNewSeller,
    overall_score: overallScore, speed_score: speedScore, pagespeed_score: pagespeedScore,
    mobile_score: mobileScore, load_speed: loadSpeed ? loadSpeed + 's' : 'Unknown',
    has_ssl: url.startsWith('https://'), redesign_needed: redesignNeeded,
    speed_estimated: speedData.estimated || false,
    ...seoData,
    products_score: productData?.products_score || 20, product_count: productCount,
    avg_desc_length: productData?.avg_desc_length || 'Unknown',
    avg_images: productData?.avg_images || 0, price_range: productData?.price_range || 'Unknown',
    best_ad_product: productData?.best_ad_product || '',
    ...trustData, ...adData, ad_platform: adPlatform, ...criticalData,
  }
}
...
async function checkSEO(base, html) {
  ...
  return { 
    seo_score: seoScore, has_meta_title: hasMetaTitle, meta_title_value: metaTitle.slice(0,60)||'Not found', 
    title_quality: titleQuality, has_meta_description: hasMetaDescription, desc_quality: descQuality, 
    has_alt_text: hasAltText, alt_text_pct: altPct+'%', has_sitemap: hasSitemap, has_robots: hasRobots,  
    has_blog: hasBlog, has_og_tags: hasOgTags, has_h1: hasH1, has_h2: hasH2, google_tier: googleTier 
  }
}
